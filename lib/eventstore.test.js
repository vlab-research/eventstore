...
describe('EventStore', () => {
  ...
  it('listens to ttl from cache', async () => {
    const db = { get: sinon.fake.returns([dat('bar'),
                                          dat('baz')]) }
    const e = new s.EventStore(db, '1s')
    await e.getEvents('foo', dat('baz'))

    db.get = sinon.fake.returns([dat('foo')])
    await sleep(1000)
    const events = await e.getEvents('foo', dat('baz'))
    events.should.deep.equal([ {foo: 'foo'}, { baz: 'baz'}])
  })
